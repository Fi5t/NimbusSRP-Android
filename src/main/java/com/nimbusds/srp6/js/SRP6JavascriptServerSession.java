 ...
 */
public class SRP6JavascriptServerSession extends SRP6JavascriptSession {
	 ...
	 */
	public SRP6JavascriptServerSession(final SRP6CryptoParams config, final int timeout) {
	 ...
	 *                be {@code null}.
	 */
	public SRP6JavascriptServerSession(final SRP6CryptoParams config) {
	...
	public BigInteger step1(final String userID, final BigInteger s, final BigInteger v) {
		...
		k = SRP6JavascriptRoutines.computeK(digest, config.N, config.g);
		digest.reset();
		
		b = SRP6JavascriptRoutines.generatePrivateValue(config.N, random);
		digest.reset();
		
		B = SRP6JavascriptRoutines.computePublicServerValue(config.N, config.g, k, v, b);
	...
	public BigInteger step2(final BigInteger A, final BigInteger M1)
		...
		// Check A validity
		if (!SRP6JavascriptRoutines.isValidPublicValue(config.N, A))
		...
		// System.out.println("jvU:" + toHex(u));

		S = SRP6JavascriptRoutines.computeSessionKey(config.N, v, u, A, b);
		...
		if (clientEvidenceRoutine != null) {
			...
			computedM1 = SRP6JavascriptRoutines.computeClientEvidence(digest, A, B, S);
			digest.reset();
		}
